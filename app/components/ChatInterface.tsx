...
const renderMessage = (message: Message) => {
  ...
  // Match code blocks of type json, xml, or markdown/md
  const match = message.content.match(/```(json|xml|markdown|md)([\s\S]*?)```/i);
  ...
  if (match) {
    ...
    const language = rawLang === 'md' ? 'markdown' : rawLang;
    const codeText = match[2].trim();

    // Remove code block from display content
    displayContent = message.content.replace(/```(json|xml|markdown|md)[\s\S]*?```/i, '').trim();

    try {
      codeComponent = (
        <CodeBlock
          code={codeText}
          language={language as 'json' | 'xml' | 'markdown'}
          title={language.toUpperCase()}
        />
      );
    } catch (error) {
      console.error(`Error rendering ${language.toUpperCase()} block:`, error);
    }
  }
